...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-14 rounded-full bg-white/50 backdrop-blur-sm border border-[#E2E2DE] px-2 ${className}`}>
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-8 h-[3px] rounded-full bg-[#E8FF3A] shadow-[0_30px_20px_#E8FF3A] ${
          isReady ? 'transition-[left] duration-500 cubic-bezier(0.16, 1, 0.3, 1)' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-50%] top-[3px] w-[200%] h-10 [clip-path:polygon(10%_100%,30%_0,70%_0,90%_100%)] bg-gradient-to-b from-[#E8FF3A]/40 to-transparent pointer-events-none blur-[2px]" />
      </div>
    </nav>
  );
};
